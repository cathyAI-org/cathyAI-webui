    ...
    <details open={isThinking} className="mt-2 mb-3 rounded-lg border border-border bg-muted/40 px-3 py-2 text-sm">
      ...
      {hasStats ? (
        <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
          {stats.prompt_tokens !== undefined ? <span>Prompt: {stats.prompt_tokens}</span> : null}
          {stats.completion_tokens !== undefined ? <span>Output: {stats.completion_tokens}</span> : null}
          {stats.total_time_s !== undefined ? <span>Total: {Number(stats.total_time_s).toFixed(2)}s</span> : null}
          {stats.load_time_s !== undefined ? <span>Load: {Number(stats.load_time_s).toFixed(2)}s</span> : null}
        </div>
      ) : null}
    </details>
  );
